...
class DENSEModule(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx):
        ...
        self.activations.extend(np.hstack((features['feats_built'].cpu().numpy(),features['feats_sat'].cpu().numpy())))
...
def predict_model(model_name,data_loaders,pretrained_filename=None, **kwargs):
    ...
    model.model.building_footprint_part.Lin2.register_forward_hook(get_features('feats_built'))
    model.model.output_net[6].register_forward_hook(get_features('feats_sat'))
...
if __name__ == "__main__":
    ...
    train_loader,val_loader,test_loader =  Dataloader_train_val_test(DATASET_PATH_pretrained,BF_DATASET_PATH_pretrained,extension,[0.1,0.2],transform,batch_size,num_workers,'test')
    ...
    np.save(r'C:\Users\JJR226\Documents\PhD\Paper3\DL_verified\classification\lightning_logs\kampala_BF300\Full_area_pred_features_s2_ori_activations.npy',np.array([y_pred,location_id,activations]))
