...
from models.Densenet42_BF import  *
...
class DENSEModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        summend_preds = torch.sum(torch.stack([preds[0]*alpha1,preds[1]*self.alpha[0],preds[2]*self.alpha[1]]), dim=0)
...
def train_model(model_name,model_hparams,optimizer_name,optimizer_hparams,weights,alpha,save_name,version_id,data_loaders,pretrained_filename,input_dict, **kwargs):
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            LearningRateMonitor("epoch"),EarlyStopping(monitor="val_loss", mode="min", patience=10)
    ...
    trainer.test(model=model, test_dataloaders=data_loaders[1])
    y_pred = model.predictions
    y_true = model.test_y  
      
    plot_cm(y_true, y_pred,save_name, version_id,data_loaders[1].dataset.dataset.classes, figsize=(8,8),dpi=100)
...
if __name__ == "__main__":
    ...
    input_variables = ['DenseNet42_Nairobi_Planet_150px_30sp_3res_Planet_BF150_300_lr2_SGD_comb2_alpha_04_valtest_0102.npy','DenseNet42_Kampala_Planet_150px_30sp_3res_Planet_BF150_300_lr2_SGD_comb2_alpha_04_valtest_0102.npy','DenseNet42_Lusaka_Planet_150px_30sp_3res_Planet_BF150__300_lr2_SGD_comb2_alpha_04_valtest_0102.npy']
    ...
    for input_file in input_variables:
        ...
        train_loader,val_loader,test_loader = Dataloader_train_val_test(DATASET_PATH,BF_DATASET_PATH,extension,val_test_size,transform,batch_size,num_workers)
        ...
        # get class weights 
        class_sample_count = np.unique(np.array(train_loader.dataset.dataset.samples)[:,1], return_counts=True)[1]
        ...
        for seed in read_dictionary['seed']:
            ...
            # train the model  
            train_model(model,
                        model_hparams={'num_input_layers':num_input_layers,'in_channels_BF':13,"num_classes": num_classes,"num_layers": num_layers,"bn_size": 2,"growth_rate": growth_rate,"act_fn_name": "relu",'lin_size':lin_size,'bin_size':bin_size},
